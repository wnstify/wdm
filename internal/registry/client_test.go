...
func TestListTags_HappyPathSortsTags(t *testing.T) {
	t.Parallel()

	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/v2/library/widget/tags/list", r.URL.Path)
		_ = json.NewEncoder(w).Encode(map[string]any{
			"name": "library/widget",
			"tags": []string{"1.2.3", "latest", "1.1.0", "", "  "},
		})
	}))
	defer srv.Close()

	c := newClientFor(t, srv)
	tags, err := c.ListTags(t.Context(), refFor(t, srv, "library/widget", "latest"))
	require.NoError(t, err)
	assert.Equal(t, []string{"1.1.0", "1.2.3", "latest"}, tags, "sorted, blanks dropped")
}

func TestListTags_MalformedJSONMapsToNetworkFailure(t *testing.T) {
	t.Parallel()
	...
	c := newClientFor(t, srv)
	_, err := c.ListTags(t.Context(), refFor(t, srv, "library/widget", "latest"))
	requireNetworkError(t, err)
}
...
func TestListTags_AnonymousTokenDance(t *testing.T) {
	...
	mux.HandleFunc("/v2/library/widget/tags/list", func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "Bearer "+issuedToken, r.Header.Get("Authorization"))
		_ = json.NewEncoder(w).Encode(map[string]any{"tags": []string{"v1", "v2"}})
	})
...
func TestListTags_MalformedReferenceMapsToUsageError(t *testing.T) {
	t.Parallel()

	c := registry.NewClient()
	_, err := c.ListTags(t.Context(), "")
	require.Error(t, err)
	assert.True(t, types.IsCode(err, types.ErrCodeUsageValidation))
}
