...
func TestAvailableApps_RealStableCatalog(t *testing.T) {
	...
	// Sorted by AppID: authentik, baserow, dockhand, docuseal, freshrss, jellyfin, meshcentral, n8n, navidrome, nextcloud, openwebui, qbittorrent, serpbear, stoat, syncthing, uptime-kuma, vaultwarden, wg-adguard, zulip.
	assert.Equal(t, []string{"authentik", "baserow", "dockhand", "docuseal", "freshrss", "jellyfin", "meshcentral", "n8n", "navidrome", "nextcloud", "openwebui", "qbittorrent", "serpbear", "stoat", "syncthing", "uptime-kuma", "vaultwarden", "wg-adguard", "zulip"}, ids)
