...
- `--auto-port` — automatically remap conflicting loopback host ports to the deterministic next-free port, non-interactively, re-planning until all conflicts clear or the install fails closed (no free port). Explicit `--port` wins: a port you remapped yourself is never auto-rescued, and only single `127.0.0.1` ports are auto-resolved (ranges and public ports stay fail-closed).
